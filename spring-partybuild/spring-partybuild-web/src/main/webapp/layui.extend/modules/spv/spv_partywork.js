layui.define(['layer', 'element', 'laydate', 'layedit', 'jquery', 'table', 'dtree', 'form'], function(exports){  
  ...
  var tableIns = table.render({
	    ...
	    even:true,
	    toolbar:'#toolbar',
	    url: '/api/v1/spv/partywork/index/view',
	    page: true ,
	    cols: [[ 
  ...
  dtree.on("node(org_tree_view)", function(obj){
	  ...
	  table.reload('mainData', {
		  url: '/api/v1/spv/partywork/ajax/getList'
		  ,where: { 
  ...
  function add(){
	  ...
	  form.on('submit(addform)', function(data){
		  $.ajax({
			  type: 'POST',  url: '/api/v1/spv/partywork/add_or_update', dataType : "json", data: data.field,
			  success: function(result) { 
